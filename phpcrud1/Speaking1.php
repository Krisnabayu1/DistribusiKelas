...
$stmt = $pdo->prepare("SELECT * FROM kontak join member on kontak.nama = member.id_kloter 
WHERE id_kloter='$nama' LIMIT  :current_page, :record_per_page");
$stmt1 = $pdo->prepare("SELECT * FROM kontak join tutor on kontak.tutor = tutor.id_tutor 
WHERE id_tutor=kontak.tutor LIMIT  :current_page, :record_per_page");
...
<div class="content read">
    ...
    <h4 style="text-align: center;"> Jam = </h4>
    ...
    <div class="pagination">
        <?php if ($page > 1) : ?>
            <a href="Speaking1.php?page=<?= $page - 1 ?>"><i class="fas fa-angle-double-left fa-sm"></i></a>
        <?php endif; ?>
        <?php if ($page * $records_per_page < $num_contacts) : ?>
            <a href="Speaking1.php?nama=<?= $page + 1 ?><?= $nama ?>"><i class="fas fa-angle-double-right fa-sm"></i></a>
        <?php endif; ?>
    </div>
</div>
